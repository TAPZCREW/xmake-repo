...
function _check_package_is_supported()
    for _, names in pairs(core_package.apis()) do
        for _, name in ipairs(names) do
            if type(name) == "string" and name == "package.on_check" then return true end
        end
    end
end
...
function _config_packages(argv, packages)
    ...
    if argv.verbose then table.insert(config_argv, "-v") end
    if argv.diagnosis then table.insert(config_argv, "-D") end
    if argv.plat then table.insert(config_argv, "--plat=" .. argv.plat) end
    if argv.arch then table.insert(config_argv, "--arch=" .. argv.arch) end
    if argv.mode then table.insert(config_argv, "--mode=" .. argv.mode) end
    if argv.policies then table.insert(config_argv, "--policies=" .. argv.policies) end
    if argv.ndk then table.insert(config_argv, "--ndk=" .. argv.ndk) end
    if argv.sdk then table.insert(config_argv, "--sdk=" .. argv.sdk) end
    if argv.ndk_sdkver then table.insert(config_argv, "--ndk_sdkver=" .. argv.ndk_sdkver) end
    if argv.vs then table.insert(config_argv, "--vs=" .. argv.vs) end
    if argv.vs_sdkver then table.insert(config_argv, "--vs_sdkver=" .. argv.vs_sdkver) end
    if argv.vs_toolset then table.insert(config_argv, "--vs_toolset=" .. argv.vs_toolset) end
    ...
    if argv.xcode_sdkver then table.insert(config_argv, "--xcode_sdkver=" .. argv.xcode_sdkver) end
    if argv.target_minver then table.insert(config_argv, "--target_minver=" .. argv.target_minver) end
    if argv.appledev then table.insert(config_argv, "--appledev=" .. argv.appledev) end
    if argv.mingw then table.insert(config_argv, "--mingw=" .. argv.mingw) end
    if argv.toolchain then table.insert(config_argv, "--toolchain=" .. argv.toolchain) end
    if argv.toolchain_host then table.insert(config_argv, "--toolchain_host=" .. argv.toolchain_host) end
    if argv.cflags then table.insert(config_argv, "--cflags=" .. argv.cflags) end
    if argv.cxxflags then table.insert(config_argv, "--cxxflags=" .. argv.cxxflags) end
    if argv.ldflags then table.insert(config_argv, "--ldflags=" .. argv.ldflags) end
...
function _get_extra_str(argv)
    local extra = {}
    if argv.mode == "debug" then extra.debug = true end
...
function _load_packages(argv, packages)
    ...
    if argv.verbose then table.insert(info_argv, "-v") end
    if argv.diagnosis then table.insert(info_argv, "-D") end
...
function _require_packages(argv, packages)
    ...
    if not argv.precompiled then table.insert(require_argv, "--build") end
    ...
    if argv.shallow or is_debug then table.insert(require_argv, "--shallow") end
    if argv.jobs then table.insert(require_argv, "--jobs=" .. argv.jobs) end
    if argv.linkjobs then table.insert(require_argv, "--linkjobs=" .. argv.linkjobs) end
    if argv.fetch then table.insert(require_argv, "--fetch") end
    local extra_str = _get_extra_str(argv)
    ...
    if _check_package_is_supported() then
        ...
        for _, package in ipairs(packages) do
            ...
            if ok == 0 then table.insert(install_packages, package) end
        end
    else
    ...
    if packages then
        ...
        for _, package in ipairs(packages_plat) do
            if package and packagename:split("%s+")[1] == package.name then
                ...
                if not arch and plat ~= os.subhost() then arch = table.wrap(platform.archs(plat))[1] end
                if not arch then arch = os.subarch() end
                for _, package_arch in ipairs(package.archs) do
                    print(package_arch, package.archs)
                    if arch == package_arch then return true end
                end
            end
        end
    end
...
function main(...)
    ...
    if #packages == 0 then packages = get_modified_packages() end
    if #packages == 0 then table.insert(packages, "tbox dev") end
    ...
    if os.isdir("xmake-repo") then os.execv(os.programfile(), { "service", "--disconnect" }) end
    if argv.remote then
        os.tryrm("xmake-repo")
    ...
    for idx, package in irpairs(packages) do
        ...
        if not _package_is_supported(argv, package) then table.remove(packages, idx) end
    end
    os.cd(old_dir)
