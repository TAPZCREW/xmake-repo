...
add_versions("0.707", "37c25566281ba9c47755be300e36e40fdb941ca286e1b03f726a299ba0bae105")
...
add_configs("static_crt", { description = "Build with static crt.", default = false, type = "boolean" })
add_configs("build_cli", { description = "Build cli.", default = false, type = "boolean" })
...
on_install(function(package)
    ...
    table.insert(configs, "-DLUAU_BUILD_SHARED=" .. (package:config("shared") and "ON" or "OFF"))
    table.insert(configs, "-DLUAU_BUILD_CLI=" .. (package:config("build_cli") and "ON" or "OFF"))
    table.insert(
        configs,
    ...
    table.insert(configs, "-DLUAU_STATIC_CRT=" .. (package:config("static_crt") and "ON" or "OFF"))
    ...
    for library_name, library_type in cmake_file:gmatch("add_library%(([%a|%.]+) (%w+)") do
        ...
        if library_name:startswith("Luau.") and (library_type == "static" or library_type == "interface") then
            if library_name:endswith(".lib") then library_name = library_name:sub(1, -5) end
            if library_type == "static" then table.insert(links, library_name) end
