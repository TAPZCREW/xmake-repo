...
package("libxcb")
    ...
    on_install("macosx", "linux", "bsd", "cross", function (package)
        ...
        local envs = import("package.tools.autoconf").buildenvs(package, {cflags = cflags, shflags = shflags})
        -- if package:is_plat("linux") then
        --     for _, dep in ipairs({"libxau", "libxdmcp", "xcb-proto"}) do
        --         local libxau = package:dep(dep)
        --         local fetchinfo = libxau:fetch()
        --         print(fetchinfo)
        --         for _, linkdir in ipairs(fetchinfo.linkdirs) do
        --             local pkgconfdir = path.join(linkdir, "pkgconfig")
        --             print("KLSJQLKJQLKSDJKQKLDJKLQJSKDLJLKQSD", pkgconfdir, os.isdir(pkgconfdir))
        --             if os.isdir(pkgconfdir) then
        --                 if not envs.PKG_CONFIG_PATH then
        --                     envs.PKG_CONFIG_PATH = pkgconfdir
        --                 else
        --                     envs.PKG_CONFIG_PATH = envs.PKG_CONFIG_PATH .. ":" .. pkgconfdir
        --                 end
        --             end
        --         end
        --     end
